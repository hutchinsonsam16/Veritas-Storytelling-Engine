...
const PanelOrderManager = ({ order, onOrderChange }: { order: PanelId[], onOrderChange: (order: PanelId[]) => void }) => {
    ...
    const handleDragStart = (e: DragEvent<HTMLDivElement>, panelId: PanelId) => {
    ...
    const handleDrop = (e: DragEvent<HTMLDivElement>, targetPanelId: PanelId) => {
        e.preventDefault();
        if (!draggedItem || draggedItem === targetPanelId) return;
                                ...
                                <Select value={settings.theme} onValueChange={(v) => updateSettings({ theme: v as Theme })}>
                                    ...
                                    <SelectContent>
                                        {Object.keys(themes).map(themeKey => (
                                            <SelectItem key={themeKey} value={themeKey}>{themeKey.charAt(0).toUpperCase() + themeKey.replace('-', ' ').slice(1)}</SelectItem>
                                        ))}
                                    </SelectContent>
                                </Select>
                            </div>
